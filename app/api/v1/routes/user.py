...
@router.post("", response_model=ApiResponse[UserResponse], status_code=status.HTTP_201_CREATED,  summary="TEST API")
...
@router.get("/{user_id}", response_model=ApiResponse[UserResponse], summary="TEST API")
async def get_user(user_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    user = await user_service.get_user(db, user_id)
    return ApiResponse.ok(UserResponse.model_validate(user), status_code=200)


@router.patch("/{user_id}", response_model=ApiResponse[UserResponse], summary="TEST API")
...
@router.delete("/{user_id}", response_model=ApiResponse[None], summary="TEST API")
